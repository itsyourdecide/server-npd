...
identity, sessions, квоты, ресурсы, уведомления
        |
        v
Domain rules
...
- внутренний `User` как единого владельца объектов Portal;
- автоматическое назначение стартовой вычислительной квоты;
- VM и другие ресурсы, напрямую связанные с `User.id`;
- проекты и membership;
- заявки на увеличение квоты и другие ресурсы вне стартового плана;
- роли `user`, `project_lead`, `reviewer`, `operator` и `admin`;
...
Регистрация в Keycloak создаёт возможность войти в Portal. После первого входа
Portal автоматически назначает безопасную стартовую квоту, не требующую
заявки. Прямой SSH, Unix account, UID, HTCondor access, увеличение квоты и
другие расширенные полномочия могут иметь отдельную политику.
...
sessions       server-side application sessions
compute        планы, квоты и пользовательские VM
projects       проекты и membership
requests       увеличение квоты и другие исключения из базовой политики
...
## Пользователь Portal и необязательные настройки

Первый успешный вход создаёт минимальный `User`, `ExternalIdentity` и
application session. Обязательной анкеты с курсом, факультетом, возрастом,
научным руководителем или целью использования нет. Эти данные не нужны для
базовых функций Portal и не собираются заранее.

```text
Keycloak authentication
        |
        v
Portal User + ExternalIdentity
        |
        v
ApplicationSession + starter quota
        |
        v
Portal
```

Имя и подтверждённый email синхронизируются из разрешённых Keycloak claims.
Язык интерфейса является необязательной настройкой `User` или безопасного
locale cookie. Принятие обязательной версии правил хранится отдельно как факт
с версией и временем, а не как академический профиль.

Если конкретная будущая функция действительно потребует организацию,
подразделение или цель использования, она запросит только необходимые ей
данные в своём workflow. Такие данные не являются условием входа или получения
стартовой квоты.
...
Portal -> create/find User and ExternalIdentity
```
...
7. Portal проверяет token и создаёт или находит внутренний `User`.
8. Новому пользователю автоматически назначается стартовая квота.
9. Portal создаёт application session и открывает пользовательский интерфейс.
...
7. При первом входе Portal создаёт `User`, назначает стартовую квоту и открывает
   пользовательский интерфейс без отдельного пароля и обязательной анкеты.
...
- состояние пользователя, квоты и конкретного ресурса;
...
аутентифицированного пользователя хранится в необязательной настройке `User`;
...
Язык email берётся из настройки пользователя. Поддерживаемые значения
...
- [Вычислительные квоты и VM](compute-quota-and-vm.md).
