...
Живая проверка при редакции: частичная; physical deployment limits подтверждены
владельцем кластера
...
```text
  ...
  pve01: designated ZFS/NFS head   pve02: fw01, pxe01, squid01,
  when JBOD is enabled;                    monitor01, bastion01, condor01
  replication peer
```
...
JBOD/NFS сейчас намеренно не входят в работающую конфигурацию после переезда.
Для полок нужны отдельный шкаф, направляющие и сопутствующая физическая
инфраструктура. Возврат storage выполняется только после закупок и отдельного
решения владельца; отсутствие `npddata` до этого не является инцидентом.
