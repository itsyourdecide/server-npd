...
- Живая проверка при редакции: PSU sensors проверены; физическая схема питания
  подтверждена владельцем кластера
...
Последний power/thermal baseline трёх PVE выполнялся 2026-07-10. На
2026-09-07 все три узла используют один power feed из-за нехватки PDU и
силовых кабелей, поэтому показание `PS2 Status = 0xb` ожидаемо. A/B power
redundancy сейчас не развёрнуто; это временное ограничение физической
инфраструктуры, а не подтверждённый отказ трёх вторых PSU.
...
После переезда JBOD намеренно выключены: для их возврата нужны отдельный шкаф,
направляющие и сопутствующая физическая инфраструктура. Состав закупок здесь не
определяется.
