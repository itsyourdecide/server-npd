...
- Живая проверка при редакции: HP LACP status и WireGuard `wg-site`
  проверены; контекст ограничения подтверждён владельцем кластера
...
Параллельно legacy user path поднята новая внешняя transport-связь:

```text
vpn-npd / Azure                         fw01 / OPNsense
20.215.200.4:51822/udp                 WAN за внешним NAT
wg-site 10.255.82.1/30  <---------->  wg-site 10.255.82.2/30
                                        local UDP 51823
```
...
- inter-switch LACP сейчас работает через один member link; это известное
  владельцу временное ограничение, не основной текущий блокер;
...
- После восстановления второго inter-switch member LACP снова должен
  обеспечивать переживание отказа одного линка; срок этой работы определяет
  владелец.
...
- [Azure edge и WireGuard plan](azure-edge-vpn-plan.md).
