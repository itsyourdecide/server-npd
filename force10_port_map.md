...
Версия: 1.0  
Дата: 2026-08-11  
Статус: **фактическая карта шкафа Supermicro**. VLAN-схема соответствует
`production_port_plan.md`; текущий вывод `show interfaces status`,
`show mac-address-table` и `show vlan` предоставлен из CLI `force10-sm`.

Этот документ — источник истины по факту "что физически воткнуто в Force10
сейчас". Обновлять при каждом изменении кабелей/портов, синхронно с
`cluster_operations_log.md`.
